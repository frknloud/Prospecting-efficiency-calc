...
function createDefaultMuseumSlots(): MuseumSlotSelection[] {
  return museumSlotsData.map((slot) => ({
    slotId: slot.slotId,
...
function normalizeRingSelections(
  savedSelections: Array<string | null> | undefined
): Array<string | null> {
  return Array.from(
    { length: RING_SLOT_COUNT },
    (_, index) => savedSelections?.[index] ?? null
  );
}

function loadSavedBuild(): SavedBuildState | null {
  ...
  try {
    ...
    return JSON.parse(raw) as SavedBuildState;
  } catch {
    return null;
  }
}
...
export default function App() {
  ...
  const [ringSlotLimit, setRingSlotLimit] = useState<6 | 8>(
    savedBuild?.ringSlotLimit === 6 ? 6 : 8
  );
  ...
  const [selectedRings, setSelectedRings] = useState<Array<string | null>>(
    normalizeRingSelections(savedBuild?.selectedRings)
  );

  const [selectedRingMutations, setSelectedRingMutations] = useState<Array<string | null>>(
    normalizeRingSelections(savedBuild?.selectedRingMutations)
  );
  ...
  useEffect(() => {
    const savedState: SavedBuildState = {
      ringSlotLimit,
      selectedPan,
      selectedPanEnchant,
      selectedShovel,
    ...
    try {
      ...
      // Ignore persistence errors so the calculator keeps working.
    }
  }, [
    ringSlotLimit,
    selectedPan,
